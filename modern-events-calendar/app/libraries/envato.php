...
class MEC_envato extends MEC_base
{
    ...
    public function get_MEC_info($type = 'dl')
    {
        ...
        $url  = get_home_url();
        if($type == 'remove') $verify_url = 'http://webnus.co/api/remove?id='.$code;
        elseif($type == 'dl') $verify_url = 'http://webnus.co/api/verify?id='.$code.'&url='.$url;
        elseif($type == 'version') $verify_url = 'http://webnus.co/api/version';
        else return;
