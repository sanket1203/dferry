...
class MEC_notifications extends MEC_base
{
    ...
    public function booking_notification($book_id)
    {
        ...
        // Single mail booking notification 
        
        // $user_email = $recipients[0];  
        
        // if(trim($user_email) == '' or !filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
        ...
        file_put_contents('df-booking-notification.txt',"bookid:".$book_id. "\n", FILE_APPEND); 
       ...
       //$allvalue = '';
        if(strpos($message, '%%attendee_full_info%%') !== false)
        {
            ...
            $attendee_full_info .= __('Email', 'mec').': '.((isset($attendee['email']) and trim($attendee['email'])) ? $attendee['email'] : '---')."\r\n";
             ...
             //$attendee_full_info .= __('Date1', 'mec').': '.((isset($attendee['date1']) and trim($attendee['date1'])) ? $attendee['date1'] : '---')."\r\n";
             
            // $allvalue .= ('Date1', 'mec').': '.((isset($attendee['date1']) and trim($attendee['date1'])) ? $attendee['date1'] : '---')."\r\n";
        ...
        //$message = str_replace('%%date1%%', $allvalue , $message);
    ...
    public function booking_confirmation($book_id)
    {
        ...
        file_put_contents('df-booking-confirm.txt',"bookid:".$book_id. "\n", FILE_APPEND);
    ...
    {
		//sandeep
		$post_detail=get_post($book_id);
		if(isset($post_detail->post_author) && $post_detail->post_author==0){
			return true;
		}
		///sandeep (end)
    ...
    public function content($message, $book_id)
    {
        ...
        $booker_name = get_the_title($book_id);
        ...
        */
         ...
         global $wpdb;
        $results = $wpdb->get_results( "select meta_value from $wpdb->postmeta where post_id = '".$book_id."' && meta_key = 'mec_transaction_id'", ARRAY_A );
        $transid = $results[0][meta_value];
        $rsults1 = $wpdb->get_results( "select post_id from $wpdb->postmeta where meta_key = 'mec_transaction_id' && meta_value = '".$transid."'", ARRAY_A );
    
        $mecid = $book_id;
        $orderid = $mecid - 1;
        $totalprice= get_post_meta( $orderid, '_order_total', true  );
           $price = 0;
            foreach($rsults1 as $rsults1Item)
            {
             $price += get_post_meta($rsults1Item['post_id'], 'mec_price', true ); 
             $last_id = $rsults1Item['post_id'];
            }
            
             $postdata = get_post_meta($last_id, 'mec_gateway_label', true ); 
             if($postdata == 'Pay Locally')
             {
				 $mec_transaction_total_amount = get_post_meta($rsults1Item['post_id'], 'mec_transaction_total_amount', true );  
                $message = str_replace('%%total_price%%',$mec_transaction_total_amount , $message);
             }
             else
             {
        ...
        $message = str_replace('%%name%%', $booker_name, $message);
        ...
        //*************************** get the return date *************************************
        $return_date    = get_post_meta($event_id, 'mec_end_date', true);
        $departure_hour = get_post_meta($event_id, 'mec_start_time_hour', true);
        $hours          =  sprintf("%02d", $departure_hour);
        $departure_min  = get_post_meta($event_id, 'mec_start_time_minutes', true);
        $mins           = sprintf("%02d", $departure_min);

        $departure_ampm = get_post_meta($event_id, 'mec_start_time_ampm', true);
        
        $departure_time =  $hours.':'.$mins.' '.$departure_ampm; 

        
        $message = str_replace('%%return_date%%', get_the_date('', $book_id), $message);
        
        //$message = str_replace('%%return_date%%', $return_date , $message); 
        $message = str_replace('%%departure_time%%', $departure_time , $message);

        $meta = $this->main->get_post_meta($book_id);
            
        $attendee   = isset($meta['mec_attendee']) ? $meta['mec_attendee'] : array();
        $daprt_date = $attendee['date1'];
        $retu_date = $attendee['date2'];
        $return_timer = $attendee['ret_time'];

        


        $message = str_replace('%%date1%%', $daprt_date, $message);
        $message = str_replace('%%date2%%', $retu_date, $message);
        $message = str_replace('%%return_time%%', $return_timer, $message);




        //$message = str_replace('%%return_date%%', (isset($event->date) ? 'END DATE' : "START DATE") , $message);
        //*************************************************************************************
        //************************************* Ticket type and number of ticketes *******************
        //$ticket_type_number   = get_post_meta($event_id, 'type_adult', true);  
        
        $ticket_adult = $attendee['type_adult'];
        $ticket_child_free = $attendee['type_child_free'];
        $ticket_child = $attendee['type_child'];
        $ticket_student = $attendee['type_student'];
        
        $message = str_replace('%%ticket_type_adult%%', $ticket_adult, $message);
        $message = str_replace('%%ticket_type_child_free%%', $ticket_child_free, $message);
        $message = str_replace('%%ticket_type_child%%', $ticket_child, $message);
        $message = str_replace('%%ticket_type_student%%', $ticket_student, $message);
        
        // if(empty($ticket_type_number)) {
        //     $message = str_replace('%%ticket_type%%', $ticket_type_number, $message);    
        // }else{
        //     $message = str_replace('%%ticket_type%%', $ticket_type_number, $message);
        // } 
        ...
        $event = get_term($event_id, 'mec-events-data');
        ...
        //echo "<pre>"; print_r($message); die("return date test"); 
        return $message;
    }
    ...
    public function  verification_links($book_id) 
    {
