...
$returncheck = $event->data->meta['mec_return_check'];
//~ echo '<pre>';
//~ print_r($event->date);
//~ echo '</pre>';
//~ echo '<pre>';
...
if($returncheck ==1){
    $returndatevalidation = '<script type="text/javascript">
   function checkretdate(){
   
           var flagerr = true;
           $("#return_dt").remove();$("#return_date").remove();
              if($("#current_time").val() =="Choose Sailing Time...."){
                   $("#mec_booking_message").after("<div id=\'return_dt\' class=\'mec-error\'>Please select return date and time</div>");
                   flagerr = false;
               }else{
                    $("#return_dt").remove();
               }
              ...
              if($("#input_01").val() ==""){
                   $("#mec_booking_message").after("<div id=\'return_date\' class=\'mec-error\'>Please select return date and time</div>");
                    flagerr = false;
               }else{
                    $("#return_date").remove();
               }
...
{ 
    ...
    // check return date time
    if(checkretdate() == false)
    {
        return false;
    }
    ...
    var old_val = $(".getsamevalue_old").val();
    var booker_title = $(".getbookername").val();  
    // $(".getsamevalue").val(old_val);  
    $(".getbookernameto").val(booker_title);
