...
class MEC_feature_events extends MEC_base
{
    ...
    public function meta_box_dates($post)
    {
        ...
        $return_event_check = get_post_meta($post->ID, 'mec_return_check', true);

        $return_status = get_post_meta($post->ID, 'mec_start_check', true);  
        ...
        if(trim($start_time_ampm) == '') $start_time_minutes = 'AM';
         ...
         $end_date = get_post_meta($post->ID, 'mec_end_date', true); //By SS 
        ...
        $date_start_time = get_post_meta($post->ID, 'date_start_time', true);
        $date_return_time = get_post_meta($post->ID, 'date_return_time', true);
        //~ $mec_settings_time_format = get_post_meta($post->ID, 'mec_settings_time_format', true);
    ...
    ?>
        <div class="mec-meta-box-fields" id="mec-date-time">
            ...
            <div id="mec_meta_box_date_form">
				<div class="mec-title">
					...
					<label for="mec_start_date"><?php _e('Departure Date', 'mec'); ?></label>
				</div>
				<div class="mec-form-row">
					<div class="mec-col-4" add>
					...
					<div class="mec-col-5 mec-time-picker <?php if($allday == 1) echo 'mec-util-hidden'; ?>" id="clone_me">
                        ...
                        {
							$date_start_time = json_decode($date_start_time,true);
							 
							$hoursm = $date_start_time['h'];
							$mm = $date_start_time['m'];
							$secondm = $date_start_time['ap']; 
							...
							<div class="mec-col-12">
							 <select name="mec[date][start][hourM][]" id="mec_start_hour">
							
							<?php for($i=1; $i<=$time_format; $i++){ ?>
							<option <?php if($hoursm[$j] == $i) echo 'selected="selected"'; ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>
							<?php } ?>
						</select>	<span class="time-dv">:</span>
						
						<select name="mec[date][start][minutesM][]" id="mec_start_minutes">
							<?php for($i=0; $i<=11; $i++): ?>
							<option <?php if($mm[$j] == ($i*5)) echo 'selected="selected"'; ?> value="<?php echo ($i*5); ?>"><?php echo sprintf("%02d", ($i*5)); ?></option>
							<?php endfor; ?>
						</select>
						<select name="mec[date][start][ampmM][]" id="mec_start_ampm">
							<option <?php if($secondm[$j] == 'AM') echo 'selected="selected"'; ?> value="AM"><?php _e('AM', 'mec'); ?></option>
							<option <?php if($secondm[$j] == 'PM') echo 'selected="selected"'; ?> value="PM"><?php _e('PM', 'mec'); ?></option>
						</select>
						</div>
						<?php } ?>
                    ...
                    <div class="mec-col-1">
                    <a href="javascript:void(0);" id="add_new_dynamic">Add Time</a>
                    </div>
                    <div class="mec-col-2">
                         


                         <input <?php if($return_status == '1') echo 'checked="checked"'; ?> type="checkbox" name="mec[date][return_status][]" id="mec_start_check" value='0'  /><label for="mec_start_check"><?php _e('No Return Trip available on same day', 'mec'); ?></label>
                    </div>
            ...
            <div class="mec-form-row">
                    ...
                    <input <?php if($return_event_check == '1') echo 'checked="checked"'; ?> type="checkbox" name="mec[date][return_visible]" id="mec_return_check" value="0" /><label for="mec_return_check"><?php _e('Event has a return date?', 'mec'); ?></label>
              ...
              <div id="rt_display_block" style="display:none; "> 
				
                <div class="mec-title" >
					<span class="mec-dashicons dashicons dashicons-calendar-alt"></span>
					<label for="mec_end_date"><?php _e('Return Date', 'mec'); ?></label>
				</div>
                <div class="mec-form-row">
					<div class="mec-col-4"> 
					...
					<div class="mec-col-6 mec-time-picker <?php if($allday == 1) echo 'mec-util-hidden'; ?>" id="return_time_clone">
                        ...
                        if(!empty($date_return_time))
                        {
							...
							<div class="mec-col-12">
							 ...
							 <select name="mec[date][start][hourR][]" id="mec_start_hour">
						...
						<select name="mec[date][start][minutesR][]" id="mec_start_minutes">
							<?php for($i=0; $i<=11; $i++): ?>
							<option <?php if($mm[$j] == ($i*5)) echo 'selected="selected"'; ?> value="<?php echo ($i*5); ?>"><?php echo sprintf("%02d", ($i*5)); ?></option>
							<?php endfor; ?>
						</select>
						<select name="mec[date][start][ampmR][]" id="mec_start_ampm">
							<option <?php if($secondm[$j] == 'AM') echo 'selected="selected"'; ?> value="AM"><?php _e('AM', 'mec'); ?></option>
							<option <?php if($secondm[$j] == 'PM') echo 'selected="selected"'; ?> value="PM"><?php _e('PM', 'mec'); ?></option>
						</select>
						</div>
					...
					<div class="mec-col-2" id="make_return_clone_click"><a href="javascript:void(0)"> Add Time</a></div>
				</div>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function()
            {
                    ...
                    jQuery('#make_return_clone_click').click(function(){ 
						
						var html='<div class="mec-col-10 mec-time-picker">'+
                                              '<select  name="mec[date][start][hourR][]" id="mec_start_hour"><option value="1"  selected="selected">1</option>'+
                                              ...
                                              '</select>'+
                                              '<span class="time-dv">:</span>'+
                                              '<select name="mec[date][start][minutesR][]" id="mec_start_minutes" ><option selected="selected" value="00">00</option>'+
                                              ...
                                              '</select></div>';
						jQuery('#return_time_clone').append(html); 
						});
                    jQuery('#add_new_dynamic').click(function(){ 
						
						var html='<div class="mec-col-10 mec-time-picker">'+
                                              '<select  name="mec[date][start][hourM][]" id="mec_start_hour"><option value="1"  selected="selected">1</option>'+
                                              '<option value="2">2</option>'+
                                              '<option value="3">3</option>'+
                                              '<option value="4">4</option>'+
                                              '<option value="5">5</option>'+
                                              ...
                                              '</select>'+
                                              '<span class="time-dv">:</span>'+
                                              '<select name="mec[date][start][minutesM][]" id="mec_start_minutes" ><option selected="selected" value="00">00</option>'+
                                              ...
                                              '<option selected="selected" value="PM">PM</option>'+
                                              '</select></div>';
						jQuery('#clone_me').append(html); 
						});
                    jQuery('#mec_return_check').on('change', function(){
                        
                        
                        this.value = this.checked ? 1 : 0;
                        var rt_value = this.value; 
                        if(this.checked) {
                            jQuery("#rt_display_block").css("display","block"); 
                            console.log("this is value"+rt_value);
                            

  
                        }
                        else {
                            jQuery("#rt_display_block").css("display","none");
                            console.log("this is value"+rt_value);
    ...
    public function save_event($post_id)
    {
		...
		$minuteArr = array();
		$hourArr = array(); 
		$hourArr = $_POST['mec']['date']['start']['hourM'];
		$minuteArr = $_POST['mec']['date']['start']['minutesM'];
		$appmArr = $_POST['mec']['date']['start']['ampmM'];
		
		$date_start_time_arr =array (
		 'h' => $hourArr,
		 'm'=>$minuteArr,
		 'ap'=>$appmArr);
		 
		$date_start_time = json_encode($date_start_time_arr); 
		
		$hourArrR = array();
		$minuteArrR = array();
		$hourArrR = array(); 
		$hourArrR = $_POST['mec']['date']['start']['hourR'];
		$minuteArrR = $_POST['mec']['date']['start']['minutesR'];
		$appmArrR = $_POST['mec']['date']['start']['ampmR'];
		 $date_return_time_arr =array (
		 'h' => $hourArrR,
		 'm'=>$minuteArrR,
		 'ap'=>$appmArrR);
		 
		$date_return_time = json_encode($date_return_time_arr); 
		//~ echo '<pre>';
		//~ print_r($date_start_time);
		//~ echo '</pre>';
		  //~ 
        ...
        $mec_return_check = isset($_mec['date']['return_visible']) ? 1 : 0;
        update_post_meta($post_id, 'mec_return_check', $mec_return_check);
        
        $date_start_time = isset($date_start_time) ? $date_start_time : 0;
        update_post_meta($post_id, 'date_start_time', $date_start_time);
        
        $date_return_time = isset($date_return_time) ? $date_return_time : 0;
        update_post_meta($post_id, 'date_return_time', $date_return_time);
        
        $mec_return_status = isset($_mec['date']['return_status']) ? 1 : 0;
        update_post_meta($post_id, 'mec_start_check', $mec_return_status);
        ...
        $more_info = isset($_mec['more_info']) ? $_mec['more_info'] : '';
        ...
        if($mec_return_status == 1) {
            //$a = date('Y-m-d', strtotime('+1 day', strtotime($start_date)));  
        ...
        $event = array('post_id'=>$post_id, 'start'=>$start_date, 'repeat'=>$repeat_status, 'rinterval'=>(!in_array($repeat_type, array('daily', 'weekly')) ? NULL : $repeat_interval), 'time_start'=>$day_start_seconds, 'time_end'=>$day_end_seconds,'return_status'=>$mec_return_check, 'return_date_status'=>$mec_return_status,'date_start_time'=>$date_start_time,'date_return_time'=>$date_return_time );  
        $year = NULL;
        $month = NULL;
        $day = NULL;
        $week = NULL;
    ...
    public function add_bulk_actions()
    {
        ...
        if($post_type == $this->PT)
        {
            ...
            jQuery(document).ready(function()
            {
                ...
                jQuery('<option>').val('sync-events').text('<?php echo __('Sync Events', 'mec'); ?>').appendTo("select[name='action']");
                jQuery('<option>').val('sync-events').text('<?php echo __('Sync Events', 'mec'); ?>').appendTo("select[name='action2']");
            });
            </script>
            <?php
        }
        ...
        switch($action)
        {
            case 'sync-events':
                ...
                global $wpdb;
                 ...
                 $sp_sourcedb = get_option('sp_sourcedb');
                 $sp_targetdb  = get_option('sp_targetdb');
                 $sp_sourceurl  = get_option('sp_sourceurl');
                 $sp_targeturl  = get_option('sp_targeturl');
                 
                 $subcon = mysqli_connect('localhost','rhcyidmx_heavent','secure2010%%',$sp_targetdb);
                 
                 $multipleIds =  implode(",",$post_ids); 
                  foreach($post_ids as $post_id){
                      $query1 = "INSERT INTO `".$sp_targetdb."`.`90sik_posts` SELECT   
 (SELECT MAX(`ID`)+1 FROM `".$sp_targetdb."`.`90sik_posts`) AS ID ,`post_author`,`post_date`,`post_date_gmt`,`post_content`,`post_title`,`post_excerpt`,
 ...
 `post_parent`,replace( `guid`,'".$sp_sourceurl."','".$sp_targeturl."'),
 `menu_order`,`post_type`,`post_mime_type`,`comment_count`
 FROM `".$sp_sourcedb."`.`90sik_posts`
 where post_type = 'mec-events' and id = '".$post_id."' " ;
    
                    $insert1 = mysqli_query($subcon,$query1);
                    $last_id = $subcon->insert_id;
                    
                    
                     
                    $query2 = "INSERT  INTO  `".$sp_targetdb."`.`90sik_postmeta` (post_id,meta_key,meta_value)  SELECT '".$last_id."' as post_id,
                                    pm.meta_key, pm.meta_value  FROM  `".$sp_sourcedb."`.`90sik_posts` p join  `".$sp_sourcedb."`.`90sik_postmeta`
                    pm on  p.ID  =  pm.post_id  and  p.id  = ".$post_id."  and pm.post_id = ".$post_id." and  p.post_type  =  'mec-events' ";
                    $insert2 = mysqli_query($subcon,$query2);
                     printf("error: %s\n", mysqli_error($subcon));
                  }
                  
                    return "Event(s) Sync successfully.";
