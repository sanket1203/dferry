...
class MEC_gateway_stripe extends MEC_gateway
{
    ...
    public function do_transaction($transaction_id = NULL)
    {
        ...
        $this->response(array('success'=>1, 'message'=>$this->main->m('book_success_message', __('Thanks for your booking. Your tickets booked, booking verification might be needed, please check your email.', 'mec')), 'data'=>array('book_id'=>$book_id, 'redirect_to'=>$redirect_to)));
    }
...
class MEC_gateway_pay_locally extends MEC_gateway
{
    ...
    ?>
        ...
        <form id="mec_do_transaction_pay_locally_form<?php echo $transaction_id; ?>">
            ...
            <input type="hidden" name="transaction_id" value="<?php echo $transaction_id; ?>" />
            <input type="hidden" name="gateway_id" value="<?php echo $this->id(); ?>" />
            <input type="hidden" name="mec_transaction_total_amount" value="<?php echo $params['amount']; ?>" />
            <?php wp_nonce_field('mec_transaction_form_'.$transaction_id); ?>
    ...
    public function do_transaction($transaction_id = NULL)
    {
        ...
        if(!trim($mec_transaction_total_amount)) $mec_transaction_total_amount = isset($_GET['mec_transaction_total_amount']) ? sanitize_text_field($_GET['mec_transaction_total_amount']) : 0;
        ...
        foreach($attendees as $attendee)
        {
            ...
            update_post_meta($book_id, 'mec_gateway_label', $this->label());
            update_post_meta($book_id, 'mec_company_book', "Doolin");
            add_post_meta($book_id, 'mec_transaction_total_amount', $mec_transaction_total_amount);
...
class MEC_gateway_woocommerce extends MEC_gateway
{
    ...
    public function create_order()
    {
        ...
        //post email data
        //$email_name = $transaction['tickets'][0]['email'];
         $emailAddress = isset($transaction['tickets']) ? $transaction['tickets'][0]['email'] : '';
         if(trim($ex[0])) update_post_meta($order->id, '_billing_email', $emailAddress);
        ...
        $this->response(array('success'=>1, 'message'=>__('Thank you! Please wait while we redirect you to our secure checkout....', 'mec'), 'data'=>array('url'=>$url, 'id'=>$order->id)));
    }
    ...
    public function do_transaction($transaction_id = NULL, $order_id = NULL)
    {
        ...
        return array('success'=>1, 'message'=>$this->main->m('book_success_message', __('Thanks for your booking. Your tickets booked, booking verification might be needed, please check your email.', 'mec')), 'data'=>array('book_id'=>$book_id, 'redirect_to'=>$redirect_to));
    }
            ...
            jQuery.ajax(
            {
                ...
                success: function(data)
                {
                    if(data.success == 1)
                    {
						...
						setTimeout(function(){window.location.href = data.data.url;}, 3000);
                        // Run the Payment Checker function after 15 seconds
                        setTimeout(function(){mec_woocommerce_pay_checker(transaction_id)}, 150);
                    }
                    else
                    {
                        // Remove the loading Class to the button
