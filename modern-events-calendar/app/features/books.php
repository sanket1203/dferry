...
class MEC_feature_books extends MEC_base
{
    ...
    public function meta_box_booking($post)
    {
        ...
        $event_namedoolin = get_post_meta($post->ID, 'mec_event_name', true);
        $company_name = get_post_meta($post->ID, 'mec_company_book', true);
            
        if($company_name =='Obrienline'){
            if($event_namedoolin ==""){
                $event_namedoolin = "-";
            }else{
                $event_namedoolin = $event_namedoolin;
            }
            $title = $event_namedoolin;
        }else{
            $title = $title;
        }
    ?>
        <div class="mec-book-details">
            ...
            <div class="mec-row">
                ...
                <?php if($company_name =='Obrienline'){ ?>
                        <span><?php echo $event_namedoolin; ?></span>
                <?php }else{ ?>
    ...
    public function filter_columns($columns)
    {
        ...
        $columns['departuredate'] = __('Departure Date', 'mec');
        $columns['company'] = __('Company', 'mec');
    ...
    public function filter_sortable_columns($columns)
    {
        ...
        $columns['bdate'] = 'date';
        $columns['departuredate'] = 'date';
        $columns['company'] = 'mec_company_book';
    ...
    public function filter_columns_content($column_name, $post_id)
    {
        if($column_name == 'event')
        {
            ...
            $event_namedoolin = get_post_meta($post_id, 'mec_event_name', true);
            $company_name = get_post_meta($post_id, 'mec_company_book', true);
            ...
            if($company_name =='Obrienline'){    
                echo $event_namedoolin;
            }else{
                 update_post_meta($post_id, 'mec_company_book', "Doolin");
        ...
        elseif($column_name == 'departuredate')
        {
			$booking_meta_data = get_post_meta($post_id);    
					$serializeArr = unserialize($booking_meta_data['mec_attendee'][0]);  
                    if(!empty($serializeArr['date1'])){
                    $departuredate=$serializeArr['date1'];
                    }
                    else
                    { 
                    $departuredate=$serializeArr['date2'];
					}
					
			update_post_meta($post_id, 'Daprt_date', $serializeArr['date1'] );
			
            echo $departuredate;
        }elseif($column_name == 'company'){
            $booking_companyname = get_post_meta($post_id,'mec_company_book',true);
            if(!empty($booking_companyname)){
                echo $booking_companyname;
            }else{
                echo 'Doolin';
            }
        }
    ...
    public function filter_query($query)
    {
        ...
        //~ elseif($orderby == 'departuredate')
        //~ {
            //~ $query->set('meta_key', 'Daprt_date');
            //~ $query->set('orderby', 'meta_value_num');
            //~ 
        //~ }
        
        elseif($orderby == 'mec_company_book')
        {
            $query->set('meta_key', 'mec_company_book');
            $query->set('orderby', 'mec_company_book');
        }
        // Meta Query
        $meta_query = array();
        ...
        //~ if(isset($_GET['departuredate']) and trim($_GET['departuredate']) != '')
        //~ {
            //~ $meta_query[] = array(
                //~ 'key'=>'Daprt_date',
                //~ 'value'=>sanitize_text_field($_GET['Daprt_date']), 
                //~ 'type'=>'date'
            //~ );
        //~ }
        
        // Filter by Verification
        //~ if(isset($_GET['mec_attendee']) and trim($_GET['mec_attendee']) != '')
        //~ {
			//~ $data = unserialize($_GET['mec_attendee'][0]);
            //~ $meta_query[] = array(
                //~ 'key'=>'mec_attendee',
                //~ 'value'=>$data['date']
            //~ );
        //~ }
        if(isset($_GET['mec_company_book']) and trim($_GET['mec_company_book']) != '')
        {
            $meta_query[] = array(
                'key'=>'mec_company_book',
                'value'=>sanitize_text_field($_GET['mec_company_book']),
                'compare'=>'=',
                'type'=>'numeric'
            );
        }
    ...
    public function do_bulk_actions()
    {
        ...
        switch($action)
        {
            ...
            case 'csv-export':
                ...
                $reg_fields = $this->main->get_reg_fields();
                $columns = array(__('ID', 'mec'), __('Event', 'mec'), __('Date', 'mec'), __('Departure Date', 'mec'), __('Ticket', 'mec'), __('Name', 'mec'), __('Email', 'mec'), __('Tel', 'mec'), __('Confirmed', 'mec'), __('Verified', 'mec'), __('Company Name', 'mec'));
                ...
                { 
                    ...
                    $event_id = get_post_meta($post_id, 'mec_event_id', true);
                    $daprt_date = get_post_meta($post_id, 'daprt_date', true);
                    $ticket_id = get_post_meta($post_id, 'mec_ticket_id', true);
                    $booking_meta_data = get_post_meta($post_id);    
					$serializeArr = unserialize($booking_meta_data['mec_attendee'][0]); 
                    //~ $mec_date = get_post_meta($post_id, 'mec_date', true);
                    //~ $mec_date_array = explode(":",$mec_date);
                    if(!empty($serializeArr['date1'])){
                    $departuredate=$serializeArr['date1'];
                    }
                    else
                    ...
                    $booker_name = get_the_title($post_id);
                    ...
                    $companyname = get_post_meta($post_id, 'mec_company_book', true);
                    if($companyname ==""){
                        $companyname = 'Doolin';
                    }else{
                   ...
                   // $booking = array($post_id, get_the_title($event_id), get_the_date('', $post_id),$departuredate, $ticket_id, (isset($booker->first_name) ? trim($booker->first_name.' '.$booker->last_name) : ''), (isset($booker->user_email) ? $booker->user_email : ''), $phone, $confirmed, $verified,$companyname);
                   
                    $event_namedoolin = get_post_meta($post_id, 'mec_event_name', true);
                    
                    if($companyname =='Obrienline'){
                        if($event_namedoolin ==""){
                            $event_namedoolin = "-";
                        }else{
                            $event_namedoolin = $event_namedoolin;
                        }
                        $titleevent = $event_namedoolin;
                    }else{
                        $titleevent = get_the_title($event_id);
                    }
                        
                        

                    $booking = array($post_id, $titleevent, get_the_date('', $post_id),$departuredate, $ticket_id, trim($booker_name), (isset($booker->user_email) ? $booker->user_email : ''), $phone, $confirmed, $verified,$companyname);
            ...
            case 'ms-excel-export':
                ...
                $reg_fields = $this->main->get_reg_fields();
                $columns = array(__('ID', 'mec'), __('Event', 'mec'), __('Date', 'mec'), __('Ticket', 'mec'), __('Name', 'mec'), __('Email', 'mec'), __('Tel', 'mec'), __('Confirmed', 'mec'), __('Verified', 'mec'), __('Company Name', 'mec'));
                foreach($reg_fields as $reg_field) $columns[] = __($reg_field['label'], 'mec');
                ...
                foreach($post_ids as $post_id)
                {
                    ...
                    $booking = array($post_id, $titleevent, get_the_date('', $post_id), $ticket_id, trim($booker_name), (isset($booker->user_email) ? $booker->user_email : ''), $phone, $confirmed, $verified,$companyname);
                    $attendee = get_post_meta($post_id, 'mec_attendee', true);
