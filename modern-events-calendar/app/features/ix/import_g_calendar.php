            ...
            <div class="mec-google-import">
                <form id="mec_google_import_form" action="<?php echo $this->main->get_full_url(); ?>" method="POST">
                    ...
                    <div class="mec-form-row">
                        ...
                        <div class="mec-col-4">
                            <input type="text" id="mec_ix_google_import_start_date" name="ix[google_import_start_date]" value="<?php echo date('Y-m-d', strtotime('-1 Month')); ?>" class="mec_date_picker" />
                        </div>
                    </div>
                    ...
                    <form id="mec_google_import_do_form" action="<?php echo $this->main->get_full_url(); ?>" method="POST">
                        <div class="mec-xi-google-import-events mec-options-fields">
                            ...
                            <ul id="mec_import_g_calendar_events">
                                <?php foreach($this->response['data']['events'] as $event): if(trim($event['title']) == '') continue; ?>
                                <li>
                                    <label>
                                        <input type="checkbox" name="g-events[]" value="<?php echo $event['id']; ?>" checked="checked" />
                                        <span><?php echo sprintf(__('Event Title: %s Event Start Date: %s', 'mec'), '<strong>'.$event['title'].'</strong>', '<strong>'.(trim($event['start']->date) ? $event['start']->date : date('Y-m-d H:i:s', strtotime($event['start']->dateTime))).'</strong>'); ?></span>
                                    </label>
                                </li>
                                <?php endforeach; ?>
                            </ul>
